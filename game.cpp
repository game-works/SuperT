...
{
//    setCacheMode(QGraphicsView::CacheBackground); TODO check if this is necessary or perfomance enhancement
    ...
    connect(this, &Game::pointIncrease, level_.data(), &LevelManager::processPlayerPoints);

    connect(menu_.data(), &Menu::start, this, &Game::play);
    connect(menu_.data(), &Menu::showSignal, this, &Game::startDemo);
...
void Game::startDemo()
{
    player_->setPosition(PLAYER_START_POS);
    hud_->show(false);
    state_ = GS_DEMO;
}
                ...
                if(e->enemy() && e->collidable() && e->destructible())
                {
                    ...
                    while(eIt.hasNext())
                    {
                                ...
                                !bullet->enemy()) //enemy hit by a shot
                        {
                            ...
                            {
                                emit pointIncrease(++playerPoints_);
                                if(playerPoints_ % 100 == 0) //TODO move this logic to level manager ?!
                                {
                                    playerSpecials_++;
                                }
                            }
